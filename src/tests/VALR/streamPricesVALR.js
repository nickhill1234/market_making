...
function streamPrices(config, handlePriceUpdateVALR, attempt = 0) {
    ...
    const resetTimeout = () => {
        ...
        timeoutId = setTimeout(() => {
            if (!hasReceivedInitialMessage) {
                ...
                wsMarket.terminate();
                streamPrices(config, handlePriceUpdateVALR, attempt + 1);
            }
            ...
            subscriptions: [
                {
                    ...
                    pairs: [config.primary_market, config.cancel_market]
                }
            ]
        };
