...
async function placeCloseOrder(side, dealId) {
    ...
    try {
        const response = await axios.post('https://demo-api.ig.com/gateway/deal/positions/otc', {
                dealId: dealId, // Deal ID of the position to close
        ...
        if (error.response) {
            console.error('Error closing position:', {
                ...
                status: error.response.status, // consider retry for 404 error here
...
module.exports = placeCloseOrder;
